...
class MessagePassingLayer(MessagePassing):
	def __init__(self, latent_dim):
		super().__init__(aggr="add")  # Aggregate messages using sum
...
class OurModel(nn.Module):
	def __init__(self, in_dim, latent_dim, k, connectivity_radius):
		...
		# Node encoder: MLP to transform input features into latent space
		...
		# Node decoder: MLP to predict acceleration (2D vector) from latent features
		self.node_decoder = nn.Sequential(
			nn.Linear(latent_dim, latent_dim//2),
			nn.ReLU(),
			nn.Linear(latent_dim//2, 2)  # Output is a 2D vector for acceleration
		)
